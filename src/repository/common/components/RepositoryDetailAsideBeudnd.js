...
const RepositorydeAside = (props) => {
    ...
    const logTree = (fItems, item, levels, faid, index) => {
        let newLevels = 0;
        return <Droppable droppableId={item.id} key={item.id} isCombineEnabled = {true}>
            {(provided, snapshot) => (
                <div
                ...
                >
                    ...
                    <Draggable key={item.id} draggableId={item.id} index={item.sort}>
                        {(provided, snapshot) => (
                ...
                <div className={`repository-aside ${isShowText ? "" : "repository-icon"}`}>
                    ...
                    <div className="repository-menu" onDrop={() => changeLog("nullString")} draggable="true" onDragOver={dragover}>
                        ...
                        <DragDropContext onDragEnd={(result) => onDragEnd(result)}>
                            <Droppable droppableId="respository">
                                {(provided, snapshot) => (
                                    <div
                                        {...provided.droppableProps}
                                        ref={provided.innerRef}
